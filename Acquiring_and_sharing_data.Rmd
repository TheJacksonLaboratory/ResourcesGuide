...
It is also possible to upload (push/publish) your results back to synapse, and specify its relations to other synapse entities, using the ```synStore()``` function. In the same way you upload the script that was used for generating those results and specify their relations as their provenance. This allows other users to interact with your results with synGet() as described above.  

See [here](https://www.synapse.org/#!Synapse:syn7419026/wiki/606653) for an example of how to share an analysis script and results and describe them using the Synapse provenance tool. The html report in that example includes input and output chunks that illustrate the use of ```synapser``` to download and upload data from and to Synapse.
...
[cache]
location = /tier2/carter-lab/sdata/.synapseCache
```
...
mayo <- read_csv(synapser::synGet("syn11384571")$path) %>%
  ...
  pins::pin(name="mayo_metadata_wgs_apoe_diagnosis", 
        ...
        board="sdata")
```
Please see above instructions on how to direct ```synGet()``` to download data into the secured cache folder on tier1 and tier2 instead of your home directory.
