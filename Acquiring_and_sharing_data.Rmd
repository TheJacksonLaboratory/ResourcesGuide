...
* keep data files together with their metadata and any processing scripts when applicable, either physically or with a reliable link between them (e.g., via RStudio Connect or pins).

A good way to interact with data objects is in a manner similar to working with git for developing code: "pushing" and "pulling" data objects to and from repositories from within your working directory, scripting everything, instead of copying data manually into the project folder. This not only makes work easier, but also helps maintain data provenance, and promotes transparency and reproducibility. Git repositories, however, are limited in size and are therefore not suitable for sharing large data objects and developing research projects. Instead we can use services such as Synapse and RStudio Connect and tools such as [Globus CLI](https://docs.globus.org) and the R [```pins``` package](http://pins.rstudio.com). In addition to providing a way to incorporate data transfer in the script, these tools also manage the caching and check for updates beforing re-downloading data. The caching ensures that we still have the latest version available in our system in the case that the original becomes unavailable for whatever reason.   
...
Often the best way to work with data from Synapse is to download/upload it directly from Synapse using its [R](https://r-docs.synapse.org/articles/synapser.html), [pyhton](https://python-docs.synapse.org/build/html/index.html), or [CLI](https://python-docs.synapse.org/build/html/CommandLineClient.html) clients.  
...
It is also possible to upload (push/publish) your results back to synapse, specify its relations to other synapse entities, and include the scripts that were used for generating your results from those entities. This allows other users to interact with your results with synGet() as described above.  

If you're working on tier1 or tier2 with secured and/or very heavy data you can change the cache folder to the general sdata folder so that the data remain in a secured location and you don't need to download your own copy if one is already available in that folder.
To do that, open the ```.synapseConfig``` file in your home directory and edit the caching part to look like this:
```
###########################
# Caching                 #
###########################
...
If you're working with secure data from Synapse (for example), you'd want to retrieve it from the sdata folder or directly from Synapse, and pin your results back to that folder for later use (when applicable). For this purpose, you'd want to register a new local board that points to the cache on sdata (once per session). For example, you might have a code chunk like this in your markdown file:
```{r}
pins::board_register_local(name = "sdata", cache = "/sdata/pins")
...
And to retrieve the modified file in another session:
```{r}
board_register_local(name = "sdata", cache = "/sdata/pins")
