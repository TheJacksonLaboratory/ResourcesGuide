...
pin(archetypes_meta, 
    ...
    board = "local")
```
Now I can close that session, start a new one within a different project on tier2, say ```ampad_ADvariants``` and in its Rmarkdown file I can include a chunk that retrieves that data file from the local cache, and further modify it.
