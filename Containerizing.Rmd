...
See the [Working With HPC](Working_with_hpc.html) section for how to use singularity containers on the cluster.  

Here's an example of building an R image that starts with the bioconductor base image and includes tidyverse and other packages in the container from the get go. Although ggplot2 is a dependency of tidyverse, it is a good idea to install it explicitly to ensure that its own dependencies are all installed as well.
The def file `R_bioc3_12_tidy1_3.def` can look like this:
```
bootstrap: docker
...
%post
 apt-get -y update
 R --slave -e 'install.packages(c("abind", "ggplot2", "gridExtra", "devtools", "tidyverse", "git2r"), dependencies = TRUE)'

%help
 R base with the latest bioconductor, tidyverse, devtools, and other packages, and all their dependencies as of Jan 10 2020
```
In order to build the image with the `singularity build` command we'd need root privileges on the cluster. To address that, jax RIT has created a builder that we can use without root privileges.  
...
singularity run http://s3-far.jax.org/builder/builder \
        bin/R_bioc3_12_tidy1_3.def bin/R_bioc3_12_tidy1_3.sif
singularity exec bin/R_bioc3_12_tidy1_3.sif R
```
You can now install other packages on your home directory to be available for future sessions and read/write files from/to your projects and home directories.

Coming soon: an example for submitting a job that renders an RMarkdown file.
...
[How to modifying basic R image and running as a pipeline on sumner](https://hpctalk.jax.org/t/running-first-r-pipeline-on-sumner/73)
