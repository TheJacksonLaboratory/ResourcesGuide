...
```
Here we're using the rstudio image that includes tidyverse, biocmanager, and devtools, as you can see [here](https://hub.docker.com/r/rocker/tidyverse/dockerfile). This way we don't need to worry about any system dependencies that they require. If your project uses packages that need other system dependencies you'd need to add that to the image def file and rebuild the sif file as explained [here](Containerizing.html)

Next you need to open the log file specified above under `#SBATCH --output=/home/flynnb/logs/rstudio-%j.log`  
...
- You might want to use [`renv`](https://rstudio.github.io/renv/articles/docker.html) to [manage your packages](Organizing_projects.html#Recording_the_computational_environment).  
