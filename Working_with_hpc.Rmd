...
To use an image that already contains tidyverse (and RStudio etc) from [rocker](https://hub.docker.com/r/rocker/tidyverse) and open an R session with those packages installed:
```
singularity pull --name R_tidy.sif docker://rocker/tidyverse:latest
singularity exec bin/tidyverse_3.6.1.sif R
```
In this case we can't just open an R session by calling the sif file because of how that def file is designed, so instead we do it with `exec`.  
...
Here's a similar solution put together by [Bill Flynn](https://thejacksonlaboratory.slack.com/archives/CMKS61RLM/p1610466642060300?thread_ts=1610466397.059200&cid=CMKS61RLM):  
...
Pull rstudio image from [rocker](https://hub.docker.com/r/rocker/rstudio) and launch it: 
```{bash}
singularity pull --name rstudio.simg docker://rocker/tidyverse:latest 
sbatch launch_rstudio.sbatch
```
Here we're using the rstudio image that includes tidyverse and devtools. This way we won't need to worry about any system dependencies that they require.

Next you need to open the log file generated in the script above under `#SBATCH --output=/home/flynnb/logs/rstudio-%j.log`  
...
- You might want to use [`renv`](https://rstudio.github.io/renv/articles/docker.html) to manage your packages.  
